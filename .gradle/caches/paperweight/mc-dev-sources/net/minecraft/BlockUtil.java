...
public class BlockUtil {
    public static BlockUtil.FoundRectangle getLargestRectangleAround(
        ...
        intBoundss[i] = new BlockUtil.IntBounds(
            getLimit(predicate, mutableBlockPos.set(center), direction3, secondaryMaxBlocks),
            getLimit(predicate, mutableBlockPos.set(center), direction4, secondaryMaxBlocks)
        );
        ...
        for (int m = 1; m <= i; m++) {
            ...
            intBoundss[k - m] = new BlockUtil.IntBounds(
                getLimit(predicate, mutableBlockPos.set(center).move(direction, m), direction3, intBounds.min),
                getLimit(predicate, mutableBlockPos.set(center).move(direction, m), direction4, intBounds.max)
            );
        }

        for (int n = 1; n <= j; n++) {
            BlockUtil.IntBounds intBounds2 = intBoundss[k + n - 1];
            intBoundss[k + n] = new BlockUtil.IntBounds(
                getLimit(predicate, mutableBlockPos.set(center).move(direction2, n), direction3, intBounds2.min),
                getLimit(predicate, mutableBlockPos.set(center).move(direction2, n), direction4, intBounds2.max)
            );
        }
