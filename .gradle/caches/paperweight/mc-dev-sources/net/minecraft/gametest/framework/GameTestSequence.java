...
public class GameTestSequence {
    ...
    public void thenFail(Supplier<Exception> exceptionSupplier) {
        this.events.add(GameTestEvent.create(() -> this.parent.fail(exceptionSupplier.get())));
    }
                ...
                this.parent
                    .fail(
                        new GameTestAssertException(
                            "Succeeded in invalid tick: expected " + (m + gameTestEvent.expectedDelay) + ", but current tick is " + tick
                        )
                    );
                break;
            }
