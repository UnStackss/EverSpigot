...
public class GameTestRegistry {
    ...
    private static <T extends Annotation> void registerBatchFunction(
        Method method, Class<T> clazz, Function<T, String> batchIdFunction, Map<String, Consumer<ServerLevel>> batchConsumerMap
    ) {
        T annotation = method.getAnnotation(clazz);
        if (annotation != null) {
            String string = batchIdFunction.apply(annotation);
            Consumer<ServerLevel> consumer = batchConsumerMap.putIfAbsent(string, (Consumer<ServerLevel>)turnMethodIntoConsumer(method));
            if (consumer != null) {
    ...
    public static Stream<TestFunction> getTestFunctionsForClassName(String testClass) {
        return TEST_FUNCTIONS.stream().filter(testFunction -> isTestFunctionPartOfClass(testFunction, testClass));
    }
    ...
    public static Consumer<ServerLevel> getBeforeBatchFunction(String batchId) {
        return BEFORE_BATCH_FUNCTIONS.getOrDefault(batchId, world -> {
        });
    }

    public static Consumer<ServerLevel> getAfterBatchFunction(String batchId) {
        return AFTER_BATCH_FUNCTIONS.getOrDefault(batchId, world -> {
        });
    }

    public static Optional<TestFunction> findTestFunction(String structurePath) {
        return getAllTestFunctions().stream().filter(testFunction -> testFunction.testName().equalsIgnoreCase(structurePath)).findFirst();
    }
