...
public class GameTestInfo {
    ...
    public GameTestInfo(TestFunction testFunction, Rotation rotation, ServerLevel world, RetryOptions testAttemptConfig) {
        this.testFunction = testFunction;
        this.level = world;
        this.retryOptions = testAttemptConfig;
    ...
    public GameTestInfo startExecution(int additionalExpectedStopTime) {
        this.startTick = this.level.getGameTime() + this.testFunction.setupTicks() + (long)additionalExpectedStopTime;
        this.timer.start();
        return this;
    }
    ...
    public void tick(GameTestRunner context) {
        if (!this.isDone()) {
            ...
            if (this.chunksLoaded
                ...
                if (this.ensureStructureIsPlaced()) {
                    ...
                    if (this.isDone()) {
                        if (this.error != null) {
                            this.listeners.forEach(listener -> listener.testFailed(this, context));
                        } else {
                            this.listeners.forEach(listener -> listener.testPassed(this, context));
                        }
                    }
                }
            }
        ...
        if (this.tickCount >= 0L) {
            ...
            if (this.tickCount > (long)this.timeoutTicks) {
                if (this.sequences.isEmpty()) {
                    ...
                    this.sequences.forEach(runner -> runner.tickAndFailIfNotComplete(this.tickCount));
                    if (this.error == null) {
                        this.fail(new GameTestTimeoutException("No sequences finished"));
                    }
                }
            } else {
                this.sequences.forEach(runner -> runner.tickAndContinue(this.tickCount));
            }
        }
    }
    ...
    public void setNorthWestCorner(BlockPos boxMinPos) {
        this.northWestCorner = boxMinPos;
    }
}
