...
public class ColorParticleOption implements ParticleOptions {
    ...
    public static MapCodec<ColorParticleOption> codec(ParticleType<ColorParticleOption> type) {
        return ExtraCodecs.ARGB_COLOR_CODEC.xmap(color -> new ColorParticleOption(type, color), effect -> effect.color).fieldOf("color");
    }

    public static StreamCodec<? super ByteBuf, ColorParticleOption> streamCodec(ParticleType<ColorParticleOption> type) {
        return ByteBufCodecs.INT.map(color -> new ColorParticleOption(type, color), particleEffect -> particleEffect.color);
    }
    ...
    public static ColorParticleOption create(ParticleType<ColorParticleOption> type, float r, float g, float b) {
        return create(type, FastColor.ARGB32.colorFromFloat(1.0F, r, g, b));
    }
}
