...
public interface DataComponentType<T> {
    ...
    StreamCodec<RegistryFriendlyByteBuf, DataComponentType<?>> STREAM_CODEC = StreamCodec.recursive(
        packetCodec -> ByteBufCodecs.registry(Registries.DATA_COMPONENT_TYPE)
    );
    Codec<DataComponentType<?>> PERSISTENT_CODEC = CODEC.validate(
        componentType -> componentType.isTransient()
                ? DataResult.error(() -> "Encountered transient component " + BuiltInRegistries.DATA_COMPONENT_TYPE.getKey(componentType))
    ...
    public static class Builder<T> {
        ...
        public DataComponentType.Builder<T> networkSynchronized(StreamCodec<? super RegistryFriendlyByteBuf, T> packetCodec) {
        ...
        static class SimpleType<T> implements DataComponentType<T> {
            ...
            SimpleType(@Nullable Codec<T> codec, StreamCodec<? super RegistryFriendlyByteBuf, T> packetCodec) {
                this.codec = codec;
                this.streamCodec = packetCodec;
            }
