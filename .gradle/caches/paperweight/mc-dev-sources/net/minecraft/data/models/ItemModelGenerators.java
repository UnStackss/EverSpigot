...
public class ItemModelGenerators {
    ...
    private void generateItemWithOverlay(Item armor) {
        this.generateLayeredItem(
            ModelLocationUtils.getModelLocation(armor), TextureMapping.getItemTexture(armor), TextureMapping.getItemTexture(armor, "_overlay")
        );
    }

    private void generateCompassItem(Item compass) {
        for (int i = 0; i < 32; i++) {
            if (i != 16) {
                this.generateFlatItem(compass, String.format(Locale.ROOT, "_%02d", i), ModelTemplates.FLAT_ITEM);
            }
        }
    }
    ...
    private void generateClockItem(Item clock) {
        for (int i = 1; i < 64; i++) {
            this.generateFlatItem(clock, String.format(Locale.ROOT, "_%02d", i), ModelTemplates.FLAT_ITEM);
        }
    }

    private void generateLayeredItem(ResourceLocation id, ResourceLocation layer0, ResourceLocation layer1) {
        ModelTemplates.TWO_LAYERED_ITEM.create(id, TextureMapping.layered(layer0, layer1), this.output);
    }

    private void generateLayeredItem(ResourceLocation id, ResourceLocation layer0, ResourceLocation layer1, ResourceLocation layer2) {
        ModelTemplates.THREE_LAYERED_ITEM.create(id, TextureMapping.layered(layer0, layer1, layer2), this.output);
    }

    private ResourceLocation getItemModelForTrimMaterial(ResourceLocation id, String trimMaterialName) {
        return id.withSuffix("_" + trimMaterialName + "_trim");
    }

    private JsonObject generateBaseArmorTrimTemplate(ResourceLocation id, Map<TextureSlot, ResourceLocation> textures, Holder<ArmorMaterial> armorMaterial) {
        ...
        for (ItemModelGenerators.TrimModelData trimModelData : GENERATED_TRIM_MODELS) {
            ...
            jsonObject2.addProperty("model", this.getItemModelForTrimMaterial(id, trimModelData.name(armorMaterial)).toString());
    ...
    private void generateArmorTrims(ArmorItem armor) {
        if (armor.getType().hasTrims()) {
            ResourceLocation resourceLocation = ModelLocationUtils.getModelLocation(armor);
            ResourceLocation resourceLocation2 = TextureMapping.getItemTexture(armor);
            ResourceLocation resourceLocation3 = TextureMapping.getItemTexture(armor, "_overlay");
            if (armor.getMaterial().is(ArmorMaterials.LEATHER)) {
                ModelTemplates.TWO_LAYERED_ITEM
                    .create(
                        ...
                        this.output,
                        (id, textures) -> this.generateBaseArmorTrimTemplate(id, textures, armor.getMaterial())
                    );
            } else {
            ...
            for (ItemModelGenerators.TrimModelData trimModelData : GENERATED_TRIM_MODELS) {
                String string = trimModelData.name(armor.getMaterial());
