...
public class VanillaRegistries {
    ...
    private static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderLookup.Provider wrapperLookup) {
        validateThatAllBiomeFeaturesHaveBiomeFilter(wrapperLookup.lookupOrThrow(Registries.PLACED_FEATURE), wrapperLookup.lookupOrThrow(Registries.BIOME));
    }

    public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> placedFeatureLookup, HolderLookup<Biome> biomeLookup) {
        biomeLookup.listElements().forEach(biome -> {
            ResourceLocation resourceLocation = biome.key().location();
            List<HolderSet<PlacedFeature>> list = biome.value().getGenerationSettings().features();
            list.stream().flatMap(HolderSet::stream).forEach(placedFeature -> placedFeature.unwrap().ifLeft(key -> {
                    Holder.Reference<PlacedFeature> referencex = placedFeatureLookup.getOrThrow((ResourceKey<PlacedFeature>)key);
                    if (!validatePlacedFeature(referencex.value())) {
                        Util.logAndPauseIfInIde("Placed feature " + key.location() + " in biome " + resourceLocation + " is missing BiomeFilter.biome()");
                    }
                }).ifRight(value -> {
