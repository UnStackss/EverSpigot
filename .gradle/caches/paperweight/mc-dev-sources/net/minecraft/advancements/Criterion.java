...
public record Criterion<T extends CriterionTriggerInstance>(CriterionTrigger<T> trigger, T triggerInstance) {
    ...
    private static <T extends CriterionTriggerInstance> Codec<Criterion<T>> criterionCodec(CriterionTrigger<T> criterion) {
        return criterion.codec().xmap(conditions -> new Criterion<>(criterion, (T)conditions), Criterion::triggerInstance);
    }
}
