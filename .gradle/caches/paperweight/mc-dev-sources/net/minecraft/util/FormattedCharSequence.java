...
public interface FormattedCharSequence {
    FormattedCharSequence EMPTY = visitor -> true;
    ...
    static FormattedCharSequence forward(String string, Style style) {
        return string.isEmpty() ? EMPTY : visitor -> StringDecomposer.iterate(string, style, visitor);
    }

    static FormattedCharSequence forward(String string, Style style, Int2IntFunction codePointMapper) {
        return string.isEmpty() ? EMPTY : visitor -> StringDecomposer.iterate(string, style, decorateOutput(visitor, codePointMapper));
    }

    static FormattedCharSequence backward(String string, Style style) {
        return string.isEmpty() ? EMPTY : visitor -> StringDecomposer.iterateBackwards(string, style, visitor);
    }

    static FormattedCharSequence backward(String string, Style style, Int2IntFunction codePointMapper) {
        return string.isEmpty() ? EMPTY : visitor -> StringDecomposer.iterateBackwards(string, style, decorateOutput(visitor, codePointMapper));
    }
