...
public final class IoSummary<T> {
    ...
    public IoSummary(Duration duration, List<Pair<T, IoSummary.CountAndSize>> packetsToStatistics) {
        this.recordingDuration = duration;
        this.totalCountAndSize = packetsToStatistics.stream().map(Pair::getSecond).reduce(new IoSummary.CountAndSize(0L, 0L), IoSummary.CountAndSize::add);
        this.largestSizeContributors = packetsToStatistics.stream()
    ...
    public static record CountAndSize(long totalCount, long totalSize) {
        ...
        IoSummary.CountAndSize add(IoSummary.CountAndSize statistics) {
            return new IoSummary.CountAndSize(this.totalCount + statistics.totalCount, this.totalSize + statistics.totalSize);
        }
