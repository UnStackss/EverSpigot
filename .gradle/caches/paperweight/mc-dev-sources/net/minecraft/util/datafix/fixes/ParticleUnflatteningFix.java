...
public class ParticleUnflatteningFix extends DataFix {
    ...
    private <T> Dynamic<T> updateItem(Dynamic<T> dynamic, String params) {
        ...
        if (i == -1) {
            ...
            CompoundTag compoundTag = parseTag(params.substring(i));
            if (compoundTag != null) {
    ...
    private <T> Dynamic<T> updateBlock(Dynamic<T> dynamic, String params) {
        int i = params.indexOf("[");
        Dynamic<T> dynamic2 = dynamic.emptyMap();
        if (i == -1) {
            dynamic2 = dynamic2.set("Name", dynamic.createString(NamespacedSchema.ensureNamespaced(params)));
        } else {
            dynamic2 = dynamic2.set("Name", dynamic.createString(NamespacedSchema.ensureNamespaced(params.substring(0, i))));
    ...
    private static <T> Dynamic<T> readVector(Dynamic<T> dynamic, StringReader paramsReader) throws CommandSyntaxException {
    ...
    private <T> Dynamic<T> updateDust(Dynamic<T> dynamic, String params) {
        try {
    ...
    private <T> Dynamic<T> updateDustTransition(Dynamic<T> dynamic, String params) {
        try {
            StringReader stringReader = new StringReader(params);
            Dynamic<T> dynamic2 = readVector(dynamic, stringReader);
            stringReader.expect(' ');
    ...
    private <T> Dynamic<T> updateSculkCharge(Dynamic<T> dynamic, String params) {
        try {
            StringReader stringReader = new StringReader(params);
            float f = stringReader.readFloat();
    ...
    private <T> Dynamic<T> updateVibration(Dynamic<T> dynamic, String params) {
        try {
            StringReader stringReader = new StringReader(params);
            ...
            Dynamic<T> dynamic2 = (Dynamic<T>)dynamic.createIntList(IntStream.of(Mth.floor(f), Mth.floor(g), Mth.floor(h)));
    ...
    private <T> Dynamic<T> updateShriek(Dynamic<T> dynamic, String params) {
        try {
            StringReader stringReader = new StringReader(params);
            int i = stringReader.readInt();
