...
public class EntityPaintingItemFrameDirectionFix extends DataFix {
    ...
    private Dynamic<?> doFix(Dynamic<?> entityDynamic, boolean isPainting, boolean isItemFrame) {
        if ((isPainting || isItemFrame) && entityDynamic.get("Facing").asNumber().result().isEmpty()) {
            ...
            if (entityDynamic.get("Direction").asNumber().result().isPresent()) {
                i = entityDynamic.get("Direction").asByte((byte)0) % DIRECTIONS.length;
                int[] is = DIRECTIONS[i];
                ...
                i = entityDynamic.get("Dir").asByte((byte)0) % DIRECTIONS.length;
    ...
    public TypeRewriteRule makeRule() {
        ...
        TypeRewriteRule typeRewriteRule = this.fixTypeEverywhereTyped(
            ...
            entityTyped -> entityTyped.updateTyped(
                    opticFinder,
                    type,
                    paintingTyped -> paintingTyped.update(DSL.remainderFinder(), paintingDynamic -> this.doFix(paintingDynamic, true, false))
                )
        );
        TypeRewriteRule typeRewriteRule2 = this.fixTypeEverywhereTyped(
            ...
            entityTyped -> entityTyped.updateTyped(
                    opticFinder2,
                    type2,
                    itemFrameTyped -> itemFrameTyped.update(DSL.remainderFinder(), itemFrameDynamic -> this.doFix(itemFrameDynamic, false, true))
                )
        );
