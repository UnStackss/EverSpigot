...
public class BlockStateData {
    ...
    public static void register(int oldIdAndMeta, String newStateStr, String... oldStateStrings) {
        Dynamic<?> dynamic = parse(newStateStr);
        MAP[oldIdAndMeta] = dynamic;
        ...
        for (String string : oldStateStrings) {
            Dynamic<?> dynamic2 = parse(string);
            String string2 = dynamic2.get("Name").asString("");
            ID_BY_OLD_NAME.putIfAbsent(string2, oldIdAndMeta);
            ID_BY_OLD.put(dynamic2, oldIdAndMeta);
        }
    }
    ...
    public static String upgradeBlock(String oldBlockName) {
        int i = ID_BY_OLD_NAME.getInt(oldBlockName);
        if (i >= 0 && i < MAP.length) {
    ...
    public static String upgradeBlock(int stateId) {
        if (stateId >= 0 && stateId < MAP.length) {
            Dynamic<?> dynamic = MAP[stateId];
    ...
    public static Dynamic<?> parse(String stateStr) {
        try {
            return new Dynamic<>(NbtOps.INSTANCE, TagParser.parseTag(stateStr.replace('\'', '"')));
