...
public abstract class SimpleEntityRenameFix extends EntityRenameFix {
    ...
    protected Pair<String, Typed<?>> fix(String choice, Typed<?> entityTyped) {
        Pair<String, Dynamic<?>> pair = this.getNewNameAndTag(choice, entityTyped.getOrCreate(DSL.remainderFinder()));
        return Pair.of(pair.getFirst(), entityTyped.set(DSL.remainderFinder(), pair.getSecond()));
    }

    protected abstract Pair<String, Dynamic<?>> getNewNameAndTag(String choice, Dynamic<?> entityDynamic);
}
