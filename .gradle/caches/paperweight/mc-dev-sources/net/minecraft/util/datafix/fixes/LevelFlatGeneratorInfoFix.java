...
public class LevelFlatGeneratorInfoFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped(
            "LevelFlatGeneratorInfoFix", this.getInputSchema().getType(References.LEVEL), levelTyped -> levelTyped.update(DSL.remainderFinder(), this::fix)
        );
    }
        ...
        return levelDynamic.get("generatorName").asString("").equalsIgnoreCase("flat")
            ? levelDynamic.update(
                ...
                generatorOptionsDynamic -> DataFixUtils.orElse(
                        generatorOptionsDynamic.asString().map(this::fixString).map(generatorOptionsDynamic::createString).result(), generatorOptionsDynamic
                    )
            )
            : levelDynamic;
    }
        ...
        if (generatorOptions.isEmpty()) {
            ...
            if (i >= 0 && i <= 3) {
                ...
                stringBuilder.append(StreamSupport.stream(LAYER_SPLITTER.split(string2).spliterator(), false).map(stringx -> {
                    ...
                    return (jx == 1 ? "" : jx + "*") + BlockStateData.getTag(m << 4 | o).get("Name").asString("");
