...
public class MissingDimensionFix extends DataFix {
    ...
    protected static <A> Type<Pair<A, Dynamic<?>>> fields(String field, Type<A> type) {
        return DSL.and(DSL.field(field, type), DSL.remainderType());
    }

    protected static <A> Type<Pair<Either<A, Unit>, Dynamic<?>>> optionalFields(String field, Type<A> type) {
    ...
    private <T> Dynamic<T> recreateSettings(Dynamic<T> worldGenSettingsDynamic) {
        long l = worldGenSettingsDynamic.get("seed").asLong(0L);
        return new Dynamic<>(
            worldGenSettingsDynamic.getOps(),
            WorldGenSettingsFix.vanillaLevels(worldGenSettingsDynamic, l, WorldGenSettingsFix.defaultOverworld(worldGenSettingsDynamic, l), false)
        );
    }
}
