...
public abstract class EntityRenameFix extends DataFix {
            ...
            dynamicOps -> pair -> {
                    ...
                    if (!type2.equals(pair2.getSecond().getType(), true, true)) {
                        throw new IllegalStateException(
                            String.format(Locale.ROOT, "Dynamic type check failed: %s not equal to %s", type2, pair2.getSecond().getType())
                        );
                    } else {
    ...
    protected abstract Pair<String, Typed<?>> fix(String choice, Typed<?> entityTyped);
}
