...
public class BannerPatternFormatFix extends NamedEntityFix {
    ...
    protected Typed<?> fix(Typed<?> inputTyped) {
        return inputTyped.update(DSL.remainderFinder(), BannerPatternFormatFix::fixTag);
    }
    ...
    private static Dynamic<?> fixLayer(Dynamic<?> dynamic) {
        dynamic = dynamic.renameAndFixField(
            ...
            dynamicx -> DataFixUtils.orElse(
                    dynamicx.asString().map(string -> PATTERN_ID_MAP.getOrDefault(string, string)).map(dynamicx::createString).result(), dynamicx
                )
        );
