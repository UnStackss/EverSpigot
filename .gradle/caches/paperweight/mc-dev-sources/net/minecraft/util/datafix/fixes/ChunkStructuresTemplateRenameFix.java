    ...
    private Dynamic<?> fixChildren(Dynamic<?> structureFeatureDynamic) {
        return structureFeatureDynamic.update(
            ...
            childrenDynamic -> structureFeatureDynamic.createList(
                    childrenDynamic.asStream().map(childDynamic -> this.fixTag(structureFeatureDynamic, (Dynamic<?>)childDynamic))
                )
        );
    }

    private Dynamic<?> fixTag(Dynamic<?> structureFeatureDynamic, Dynamic<?> childDynamic) {
