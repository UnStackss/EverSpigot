        ...
        Optional<? extends Dynamic<?>> optional = tagDynamic.get("ench")
            ...
            .map(
                enchantments -> enchantments.map(
                        enchantment -> enchantment.set("id", enchantment.createString(MAP.getOrDefault(enchantment.get("id").asInt(0), "null")))
                    )
            )
                        ...
                        .map(
                            storedEnchantments -> storedEnchantments.map(
                                    storedEnchantment -> storedEnchantment.set(
                                            "id", storedEnchantment.createString(MAP.getOrDefault(storedEnchantment.get("id").asInt(0), "null"))
                                        )
                                )
                        )
