...
public class AttributeModifierIdFix extends DataFix {
    ...
    private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> dynamicStream) {
        ...
        dynamicStream.forEach(modifierDynamic -> {
            UUID uUID = uuidFromIntArray(modifierDynamic.get("uuid").asIntStream().toArray());
    ...
    private static Dynamic<?> convertModifierForEntity(Dynamic<?> attributeDynamic) {
    ...
    private static Dynamic<?> fixItemStackComponents(Dynamic<?> stackDataDynamic) {
        return stackDataDynamic.update(
            ...
            attributeModifiersDynamic -> attributeModifiersDynamic.update(
                    ...
                    modifiersDynamic -> DataFixUtils.orElse(
                            modifiersDynamic.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(modifiersDynamic::createList),
                            modifiersDynamic
                        )
                )
        );
            ...
            remainder -> remainder.renameAndFixField(
                    ...
                    attributesDynamic -> DataFixUtils.orElse(
                            attributesDynamic.asStreamOpt()
                                .result()
                                .map(stream -> stream.map(AttributeModifierIdFix::fixAttribute))
