...
public class IdDispatchCodec<B extends ByteBuf, V, T> implements StreamCodec<B, V> {
    ...
    IdDispatchCodec(Function<V, ? extends T> packetIdGetter, List<IdDispatchCodec.Entry<B, V, T>> packetTypes, Object2IntMap<T> typeToIndex) {
    ...
    public static <B extends ByteBuf, V, T> IdDispatchCodec.Builder<B, V, T> builder(Function<V, ? extends T> packetIdGetter) {
        return new IdDispatchCodec.Builder<>(packetIdGetter);
    }
    ...
    public static class Builder<B extends ByteBuf, V, T> {
        ...
        public IdDispatchCodec.Builder<B, V, T> add(T id, StreamCodec<? super B, ? extends V> codec) {
            this.entries.add(new IdDispatchCodec.Entry<>(codec, id));
            return this;
        }
