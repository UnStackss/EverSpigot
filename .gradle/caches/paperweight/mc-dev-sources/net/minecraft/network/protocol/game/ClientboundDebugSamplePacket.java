...
public record ClientboundDebugSamplePacket(long[] sample, RemoteDebugSampleType debugSampleType) implements Packet<ClientGamePacketListener> {
    ...
    private ClientboundDebugSamplePacket(FriendlyByteBuf buf) {
        this(buf.readLongArray(), buf.readEnum(RemoteDebugSampleType.class));
    }

    private void write(FriendlyByteBuf buf) {
        buf.writeLongArray(this.sample);
        buf.writeEnum(this.debugSampleType);
    }
