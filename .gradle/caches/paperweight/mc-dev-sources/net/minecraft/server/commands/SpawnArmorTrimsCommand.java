...
public class SpawnArmorTrimsCommand {
    ...
    private static int spawnArmorTrims(CommandSourceStack source, Player player) {
        ...
        registry.stream()
            .sorted(Comparator.comparing(pattern -> TRIM_PATTERN_ORDER.applyAsInt(registry.getResourceKey(pattern).orElse(null))))
            .forEachOrdered(
                pattern -> registry2.stream()
                        .sorted(Comparator.comparing(material -> TRIM_MATERIAL_ORDER.applyAsInt(registry2.getResourceKey(material).orElse(null))))
                        .forEachOrdered(material -> nonNullList.add(new ArmorTrim(registry2.wrapAsHolder(material), registry.wrapAsHolder(pattern))))
            );
