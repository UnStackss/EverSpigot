...
public class StorageDataAccessor implements DataAccessor {
            ...
            public ArgumentBuilder<CommandSourceStack, ?> wrap(
                ...
                return argument.then(
                    Commands.literal("storage")
                        .then(argumentAdder.apply(Commands.argument(argumentName, ResourceLocationArgument.id()).suggests(StorageDataAccessor.SUGGEST_STORAGE)))
                );
            }
        };
    ...
    public Component getPrintSuccess(Tag element) {
        return Component.translatable("commands.data.storage.query", Component.translationArg(this.id), NbtUtils.toPrettyComponent(element));
    }

    @Override
    public Component getPrintSuccess(NbtPathArgument.NbtPath path, double scale, int result) {
        return Component.translatable(
            "commands.data.storage.get", path.asString(), Component.translationArg(this.id), String.format(Locale.ROOT, "%.2f", scale), result
        );
    }
}
