...
public class BossBarCommands {
    private static final DynamicCommandExceptionType ERROR_ALREADY_EXISTS = new DynamicCommandExceptionType(
        name -> Component.translatableEscape("commands.bossbar.create.failed", name)
    );
    private static final DynamicCommandExceptionType ERROR_DOESNT_EXIST = new DynamicCommandExceptionType(
        name -> Component.translatableEscape("commands.bossbar.unknown", name)
    );
                    ...
                    Commands.literal("remove")
                        .then(
                            Commands.argument("id", ResourceLocationArgument.id())
                                ...
                                .executes(context -> removeBar(context.getSource(), getBossBar(context)))
                        )
                )
                .then(Commands.literal("list").executes(context -> listBars(context.getSource())))
                .then(
                                    ...
                                    Commands.literal("name")
                                        .then(
                                            Commands.argument("name", ComponentArgument.textComponent(registryAccess))
                                                .executes(
                                                    context -> setName(
                                                            context.getSource(), getBossBar(context), ComponentArgument.getComponent(context, "name")
                                                        )
                                                )
                                        )
                                )
                                    ...
                                    Commands.literal("value")
                                        .then(
                                            Commands.argument("value", IntegerArgumentType.integer(0))
                                                .executes(
                                                    context -> setValue(
                                                            context.getSource(), getBossBar(context), IntegerArgumentType.getInteger(context, "value")
                                                        )
                                                )
                                        )
                                )
                                    ...
                                    Commands.literal("max")
                                        .then(
                                            Commands.argument("max", IntegerArgumentType.integer(1))
                                                .executes(
                                                    context -> setMax(context.getSource(), getBossBar(context), IntegerArgumentType.getInteger(context, "max"))
                                                )
                                        )
                                )
                                .then(
                                    Commands.literal("visible")
                                        .then(
                                            Commands.argument("visible", BoolArgumentType.bool())
                                                .executes(
                                                    context -> setVisible(
                                                            context.getSource(), getBossBar(context), BoolArgumentType.getBool(context, "visible")
                                                        )
                                                )
                                        )
                                )
                                    ...
                                    Commands.literal("players")
                                        ...
                                        .then(
                                            Commands.argument("targets", EntityArgument.players())
                                                .executes(
                                                    context -> setPlayers(
                                                            context.getSource(), getBossBar(context), EntityArgument.getOptionalPlayers(context, "targets")
                                                        )
                                                )
                                        )
                                )
                ...
                .then(
                    Commands.literal("get")
                        .then(
                            Commands.argument("id", ResourceLocationArgument.id())
                                .suggests(SUGGEST_BOSS_BAR)
                                .then(Commands.literal("value").executes(context -> getValue(context.getSource(), getBossBar(context))))
                                .then(Commands.literal("max").executes(context -> getMax(context.getSource(), getBossBar(context))))
                                .then(Commands.literal("visible").executes(context -> getVisible(context.getSource(), getBossBar(context))))
                                .then(Commands.literal("players").executes(context -> getPlayers(context.getSource(), getBossBar(context))))
                        )
                )
        );
    }
