...
public class TitleCommand {
            ...
            Commands.literal("title")
                ...
                .then(
                    Commands.argument("targets", EntityArgument.players())
                        .then(Commands.literal("clear").executes(context -> clearTitle(context.getSource(), EntityArgument.getPlayers(context, "targets"))))
                        .then(Commands.literal("reset").executes(context -> resetTitle(context.getSource(), EntityArgument.getPlayers(context, "targets"))))
                        .then(
                            Commands.literal("title")
                                .then(
                                    Commands.argument("title", ComponentArgument.textComponent(registryAccess))
                                        .executes(
                                            context -> showTitle(
                                                    context.getSource(),
    ...
    private static int showTitle(
        CommandSourceStack source, Collection<ServerPlayer> targets, Component title, String titleType, Function<Component, Packet<?>> constructor
