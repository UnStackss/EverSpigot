...
public class BlockDataAccessor implements DataAccessor {
    ...
    public void setData(CompoundTag nbt) {
        ...
        this.entity.loadWithComponents(nbt, this.entity.getLevel().registryAccess());
    ...
    public Component getPrintSuccess(Tag element) {
        return Component.translatable("commands.data.block.query", this.pos.getX(), this.pos.getY(), this.pos.getZ(), NbtUtils.toPrettyComponent(element));
    }

    @Override
    public Component getPrintSuccess(NbtPathArgument.NbtPath path, double scale, int result) {
        return Component.translatable(
            "commands.data.block.get", path.asString(), this.pos.getX(), this.pos.getY(), this.pos.getZ(), String.format(Locale.ROOT, "%.2f", scale), result
        );
    }
}
