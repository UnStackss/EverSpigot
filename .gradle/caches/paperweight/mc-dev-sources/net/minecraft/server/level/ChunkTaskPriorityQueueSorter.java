...
public class ChunkTaskPriorityQueueSorter implements ChunkHolder.LevelChangeListener, AutoCloseable {
    ...
    public static <T> ChunkTaskPriorityQueueSorter.Message<T> message(
        Function<ProcessorHandle<Unit>, T> taskFunction, long pos, IntSupplier lastLevelUpdatedToProvider
    ) {
    ...
    public static ChunkTaskPriorityQueueSorter.Message<Runnable> message(GenerationChunkHolder holder, Runnable task) {
        return message(task, holder.getPos().toLong(), holder::getQueueLevel);
    }

    public static <T> ChunkTaskPriorityQueueSorter.Message<T> message(GenerationChunkHolder holder, Function<ProcessorHandle<Unit>, T> taskFunction) {
    ...
    public <T> ProcessorHandle<ChunkTaskPriorityQueueSorter.Message<T>> getProcessor(ProcessorHandle<T> executor, boolean addBlocker) {
        return this.mailbox
            .<ProcessorHandle<ChunkTaskPriorityQueueSorter.Message<T>>>ask(
                yield -> new StrictQueue.IntRunnable(
                        0,
                        () -> {
                            this.getQueue(executor);
                            yield.tell(
                                ProcessorHandle.of(
                                    "chunk priority sorter around " + executor.name(),
                                    task -> this.submit(executor, task.task, task.pos, task.level, addBlocker)
                                )
                            );
                        }
                    )
            ...
            .<ProcessorHandle<ChunkTaskPriorityQueueSorter.Release>>ask(
                yield -> new StrictQueue.IntRunnable(
                        ...
                        () -> yield.tell(
                                ProcessorHandle.of(
                                    "chunk priority sorter around " + executor.name(),
                                    message -> this.release(executor, message.pos, message.task, message.clearQueue)
                                )
                            )
                    )
            )
    ...
    private <T> void submit(
        ProcessorHandle<T> actor, Function<ProcessorHandle<Unit>, T> task, long chunkPos, IntSupplier lastLevelUpdatedToProvider, boolean addBlocker
    ) {
    ...
    private <T> void pollTask(ChunkTaskPriorityQueue<Function<ProcessorHandle<Unit>, T>> queue, ProcessorHandle<T> actor) {
        this.mailbox.tell(new StrictQueue.IntRunnable(3, () -> {
            ...
            if (stream == null) {
                ...
                CompletableFuture.allOf(stream.map(executeOrAddBlocking -> executeOrAddBlocking.map(actor::ask, addBlocking -> {
                        addBlocking.run();
        ...
        return this.queues
                ...
                .map(
                    entry -> entry.getKey().name()
                            + "=["
                            + entry.getValue().getAcquired().stream().map(pos -> pos + ":" + new ChunkPos(pos)).collect(Collectors.joining(","))
                            + "]"
                )
