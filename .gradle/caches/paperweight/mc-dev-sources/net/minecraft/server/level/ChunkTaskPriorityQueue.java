...
public class ChunkTaskPriorityQueue<T> {
    ...
    protected void resortChunkTasks(int fromLevel, ChunkPos pos, int toLevel) {
        if (fromLevel < PRIORITY_LEVEL_COUNT) {
            ...
            if (list != null && !list.isEmpty()) {
                this.taskQueue.get(toLevel).computeIfAbsent(pos.toLong(), l -> Lists.newArrayList()).addAll(list);
    ...
    protected void submit(Optional<T> element, long pos, int level) {
        this.taskQueue.get(level).computeIfAbsent(pos, l -> Lists.newArrayList()).add(element);
