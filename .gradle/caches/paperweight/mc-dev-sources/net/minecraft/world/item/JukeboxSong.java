...
public record JukeboxSong(Holder<SoundEvent> soundEvent, Component description, float lengthInSeconds, int comparatorOutput) {
    ...
    public boolean hasFinished(long ticksSinceSongStarted) {
        return ticksSinceSongStarted >= (long)(this.lengthInTicks() + 20);
    }

    public static Optional<Holder<JukeboxSong>> fromStack(HolderLookup.Provider registryLookup, ItemStack stack) {
        JukeboxPlayable jukeboxPlayable = stack.get(DataComponents.JUKEBOX_PLAYABLE);
        return jukeboxPlayable != null ? jukeboxPlayable.song().unwrap(registryLookup) : Optional.empty();
    }
}
