...
public record Tool(List<Tool.Rule> rules, float defaultMiningSpeed, int damagePerBlock) {
    ...
    public static record Rule(HolderSet<Block> blocks, Optional<Float> speed, Optional<Boolean> correctForDrops) {
        ...
        public static Tool.Rule deniesDrops(TagKey<Block> tag) {
            return forTag(tag, Optional.empty(), Optional.of(false));
        }

        public static Tool.Rule overrideSpeed(TagKey<Block> tag, float speed) {
            return forTag(tag, Optional.of(speed), Optional.empty());
        }
