...
public record WrittenBookContent(Filterable<String> title, String author, int generation, @Override List<Filterable<Component>> pages, boolean resolved)
    ...
    private static Codec<Filterable<Component>> pageCodec(Codec<Component> textCodec) {
        return Filterable.codec(textCodec);
    }

    public static Codec<List<Filterable<Component>>> pagesCodec(Codec<Component> textCodec) {
        return pageCodec(textCodec).listOf();
    }
    ...
    private static Optional<Filterable<Component>> resolvePage(CommandSourceStack source, @Nullable Player player, Filterable<Component> page) {
        return page.resolve(text -> {
            try {
                Component component = ComponentUtils.updateForEntity(source, text, player, 0);
    ...
    private static boolean isPageTooLarge(Component text, HolderLookup.Provider lookup) {
        return Component.Serializer.toJson(text, lookup).length() > 32767;
    }

    public List<Component> getPages(boolean shouldFilter) {
        return Lists.transform(this.pages, page -> page.get(shouldFilter));
    }
