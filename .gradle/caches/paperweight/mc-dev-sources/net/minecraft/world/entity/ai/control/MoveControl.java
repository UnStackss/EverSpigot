...
public class MoveControl implements Control {
    ...
    public void tick() {
        if (this.operation == MoveControl.Operation.STRAFE) {
            ...
            float q = (float)(Mth.atan2(e, d) * 180.0F / (float)Math.PI) - 90.0F;
        ...
        if (pathNavigation != null) {
            ...
            if (nodeEvaluator != null
                && nodeEvaluator.getPathType(
                        this.mob, BlockPos.containing(this.mob.getX() + (double)x, (double)this.mob.getBlockY(), this.mob.getZ() + (double)z)
                    )
                    != PathType.WALKABLE) {
                return false;
            }
