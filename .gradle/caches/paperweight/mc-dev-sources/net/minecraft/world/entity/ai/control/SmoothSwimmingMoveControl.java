        ...
        if (this.operation == MoveControl.Operation.MOVE_TO && !this.mob.getNavigation().isDone()) {
            ...
            if (g < 2.5000003E-7F) {
                ...
                if (this.mob.isInWater()) {
                    ...
                    float l = Mth.cos(this.mob.getXRot() * (float) (Math.PI / 180.0));
                    float m = Mth.sin(this.mob.getXRot() * (float) (Math.PI / 180.0));
                    this.mob.zza = l * i;
                    this.mob.yya = -m * i;
                } else {
                    float n = Math.abs(Mth.wrapDegrees(this.mob.getYRot() - h));
                    float o = getTurningSpeedFactor(n);
                    this.mob.setSpeed(i * this.outsideWaterSpeedModifier * o);
                }
            }
        } else {
