                        ...
                        pos -> {
                            if (!serverLevel.isVillage(pos)) {
                                ...
                                Optional<BlockPos> optionalx = serverLevel.getPoiManager()
                                    .find(poiType -> poiType.is(PoiTypeTags.VILLAGE), this::hasNotVisited, pos, 10, PoiManager.Occupancy.IS_OCCUPIED);
                                return optionalx.<Double>map(blockPos2x -> -blockPos2x.distSqr(blockPos)).orElse(Double.NEGATIVE_INFINITY);
                            }
                        }
                    );
                    if (vec3 == null) {
