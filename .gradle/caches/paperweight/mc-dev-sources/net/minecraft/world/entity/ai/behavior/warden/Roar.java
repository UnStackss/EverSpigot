...
public class Roar extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel world, Warden entity, long time) {
        if (!entity.getBrain().hasMemoryValue(MemoryModuleType.ROAR_SOUND_DELAY) && !entity.getBrain().hasMemoryValue(MemoryModuleType.ROAR_SOUND_COOLDOWN)) {
            entity.getBrain().setMemoryWithExpiry(MemoryModuleType.ROAR_SOUND_COOLDOWN, Unit.INSTANCE, (long)(WardenAi.ROAR_DURATION - 25));
            entity.playSound(SoundEvents.WARDEN_ROAR, 3.0F, 1.0F);
        }
    }
