...
public class CamelAi {
        ...
        brain.addActivity(
            ...
            ImmutableList.of(
                ...
                Pair.of(
                    ...
                    new RunOne<>(
                        ImmutableList.of(
                            Pair.of(new FollowTemptation(entity -> 2.5F, entity -> entity.isBaby() ? 2.5 : 3.5), 1),
    ...
    public static class RandomSitting extends Behavior<Camel> {
        ...
        @Override
        protected boolean checkExtraStartConditions(ServerLevel world, Camel entity) {
            return !entity.isInWater()
                && entity.getPoseTime() >= (long)this.minimalPoseTicks
                && !entity.isLeashed()
                && entity.onGround()
                && !entity.hasControllingPassenger()
                && entity.canCamelChangePose();
        }
