...
public abstract class PatrollingMonster extends Monster {
    ...
    public void addAdditionalSaveData(CompoundTag nbt) {
        ...
        if (this.patrolTarget != null) {
            nbt.put("patrol_target", NbtUtils.writeBlockPos(this.patrolTarget));
        }
    ...
    public void readAdditionalSaveData(CompoundTag nbt) {
        ...
        NbtUtils.readBlockPos(nbt, "patrol_target").ifPresent(patrolTarget -> this.patrolTarget = patrolTarget);
    ...
    public static boolean checkPatrollingMonsterSpawnRules(
        EntityType<? extends PatrollingMonster> type, LevelAccessor world, MobSpawnType spawnReason, BlockPos pos, RandomSource random
    ) {
        return world.getBrightness(LightLayer.BLOCK, pos) <= 8 && checkAnyLightMonsterSpawnRules(type, world, spawnReason, pos, random);
    }
            ...
            if (pathNavigation.isDone()) {
                ...
                if (this.mob.isPatrolling() && list.isEmpty()) {
                    ...
                    if (!pathNavigation.moveTo(
                        (double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ(), bl ? this.leaderSpeedModifier : this.speedModifier
                    )) {
                        this.moveRandomly();
