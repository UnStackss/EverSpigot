...
public class DismountHelper {
    public static int[][] offsetsForDirection(Direction movementDirection) {
        ...
        return new int[][]{
            ...
            {direction3.getStepX() + direction.getStepX(), direction3.getStepZ() + direction.getStepZ()},
            {direction3.getStepX() + direction2.getStepX(), direction3.getStepZ() + direction2.getStepZ()},
            {movementDirection.getStepX() + direction.getStepX(), movementDirection.getStepZ() + direction.getStepZ()},
            {movementDirection.getStepX() + direction2.getStepX(), movementDirection.getStepZ() + direction2.getStepZ()},
            {direction3.getStepX(), direction3.getStepZ()},
            {movementDirection.getStepX(), movementDirection.getStepZ()}
        };
    }
    ...
    public static Vec3 findSafeDismountLocation(EntityType<?> entityType, CollisionGetter world, BlockPos pos, boolean ignoreInvalidPos) {
        if (ignoreInvalidPos && entityType.isBlockDangerous(world.getBlockState(pos))) {
            ...
            double d = world.getBlockFloorHeight(nonClimbableShape(world, pos), () -> nonClimbableShape(world, pos.below()));
            if (!isBlockFloorValid(d)) {
                return null;
            } else if (ignoreInvalidPos && d <= 0.0 && entityType.isBlockDangerous(world.getBlockState(pos.below()))) {
                return null;
            } else {
                Vec3 vec3 = Vec3.upFromBottomCenterOf(pos, d);
                AABB aABB = entityType.getDimensions().makeBoundingBox(vec3);

                for (VoxelShape voxelShape : world.getBlockCollisions(null, aABB)) {
                    if (!voxelShape.isEmpty()) {
                        return null;
                    }
                }
                ...
                if (entityType != EntityType.PLAYER
                    || !world.getBlockState(pos).is(BlockTags.INVALID_SPAWN_INSIDE) && !world.getBlockState(pos.above()).is(BlockTags.INVALID_SPAWN_INSIDE)) {
