...
public class GoalUtils {
    ...
    public static boolean mobRestricted(PathfinderMob entity, int extraDistance) {
        return entity.hasRestriction()
            && entity.getRestrictCenter().closerToCenterThan(entity.position(), (double)(entity.getRestrictRadius() + (float)extraDistance) + 1.0);
    }
    ...
    public static boolean hasMalus(PathfinderMob entity, BlockPos pos) {
        return entity.getPathfindingMalus(WalkNodeEvaluator.getPathTypeStatic(entity, pos)) != 0.0F;
    }

    public static boolean isSolid(PathfinderMob entity, BlockPos pos) {
        return entity.level().getBlockState(pos).isSolid();
    }
}
