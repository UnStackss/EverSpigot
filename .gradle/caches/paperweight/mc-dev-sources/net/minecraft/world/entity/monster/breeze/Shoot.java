...
public class Shoot extends Behavior<Breeze> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel world, Breeze entity) {
        return entity.getPose() == Pose.STANDING
            && entity.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).map(target -> isTargetWithinRange(entity, target)).map(withinRange -> {
                if (!withinRange) {
    ...
    public static boolean isFacingTarget(Breeze breeze, LivingEntity target) {
        ...
        Vec3 vec32 = target.position().subtract(breeze.position()).normalize();
