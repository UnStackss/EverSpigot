...
public class LandRandomPos {
        ...
        return RandomPos.generateRandomPos(
            ...
            () -> {
                BlockPos blockPos = RandomPos.generateRandomDirectionWithinRadians(
                    entity.getRandom(), horizontalRange, verticalRange, 0, direction.x, direction.z, (float) (Math.PI / 2)
                );
                if (blockPos == null) {
                    return null;
                } else {
    ...
    public static BlockPos movePosUpOutOfSolid(PathfinderMob entity, BlockPos pos) {
        pos = RandomPos.moveUpOutOfSolid(pos, entity.level().getMaxBuildHeight(), currentPos -> GoalUtils.isSolid(entity, currentPos));
    ...
    public static BlockPos generateRandomPosTowardDirection(PathfinderMob entity, int horizontalRange, boolean posTargetInRange, BlockPos relativeInRangePos) {
        ...
        return !GoalUtils.isOutsideLimits(blockPos, entity)
                && !GoalUtils.isRestricted(posTargetInRange, entity, blockPos)
