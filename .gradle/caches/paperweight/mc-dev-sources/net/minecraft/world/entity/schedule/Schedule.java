...
public class Schedule {
    ...
    protected static ScheduleBuilder register(String id) {
        Schedule schedule = Registry.register(BuiltInRegistries.SCHEDULE, id, new Schedule());
    ...
    protected List<Timeline> getAllTimelinesExceptFor(Activity activity) {
        return this.timelines.entrySet().stream().filter(rule -> rule.getKey() != activity).map(Entry::getValue).collect(Collectors.toList());
    }
    ...
    public Activity getActivityAt(int time) {
        return this.timelines
            .entrySet()
            .stream()
            .max(Comparator.comparingDouble(rule -> (double)rule.getValue().getValueAt(time)))
            .map(Entry::getKey)
