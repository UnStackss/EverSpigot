...
public class NumberProviders {
    ...
    public static final Codec<NumberProvider> CODEC = Codec.lazyInitialized(
        () -> {
            ...
            return Codec.either(ConstantValue.INLINE_CODEC, codec)
                .xmap(Either::unwrap, provider -> provider instanceof ConstantValue constantValue ? Either.left(constantValue) : Either.right(provider));
        }
    );
    ...
    private static LootNumberProviderType register(String id, MapCodec<? extends NumberProvider> codec) {
        return Registry.register(BuiltInRegistries.LOOT_NUMBER_PROVIDER_TYPE, ResourceLocation.withDefaultNamespace(id), new LootNumberProviderType(codec));
    }
}
