...
public record DimensionType(
    ...
    public static Path getStorageFolder(ResourceKey<Level> worldRef, Path worldDirectory) {
        if (worldRef == Level.OVERWORLD) {
            ...
            return worldRef == Level.NETHER
                ? worldDirectory.resolve("DIM-1")
                : worldDirectory.resolve("dimensions").resolve(worldRef.location().getNamespace()).resolve(worldRef.location().getPath());
        }
    }
    ...
    public float timeOfDay(long time) {
        double d = Mth.frac((double)this.fixedTime.orElse(time) / 24000.0 - 0.25);
