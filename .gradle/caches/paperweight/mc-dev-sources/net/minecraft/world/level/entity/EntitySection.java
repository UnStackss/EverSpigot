...
public class EntitySection<T extends EntityAccess> {
    ...
    public <U extends T> AbortableIterationConsumer.Continuation getEntities(
        EntityTypeTest<T, U> type, AABB box, AbortableIterationConsumer<? super U> consumer
    ) {
        ...
        if (collection.isEmpty()) {
            ...
            for (T entityAccess : collection) {
                U entityAccess2 = (U)type.tryCast(entityAccess);
                if (entityAccess2 != null && entityAccess.getBoundingBox().intersects(box) && consumer.accept(entityAccess2).shouldAbort()) {
                    return AbortableIterationConsumer.Continuation.ABORT;
                }
            }
