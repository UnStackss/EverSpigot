...
public class FossilFeatureConfiguration implements FeatureConfiguration {
    public static final Codec<FossilFeatureConfiguration> CODEC = RecordCodecBuilder.create(
        instance -> instance.group(
                    ResourceLocation.CODEC.listOf().fieldOf("fossil_structures").forGetter(config -> config.fossilStructures),
                    ResourceLocation.CODEC.listOf().fieldOf("overlay_structures").forGetter(config -> config.overlayStructures),
                    StructureProcessorType.LIST_CODEC.fieldOf("fossil_processors").forGetter(config -> config.fossilProcessors),
                    StructureProcessorType.LIST_CODEC.fieldOf("overlay_processors").forGetter(config -> config.overlayProcessors),
                    Codec.intRange(0, 7).fieldOf("max_empty_corners_allowed").forGetter(config -> config.maxEmptyCornersAllowed)
                )
    ...
    public FossilFeatureConfiguration(
        List<ResourceLocation> fossilStructures,
        List<ResourceLocation> overlayStructures,
