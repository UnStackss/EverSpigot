...
public class LootContextParamSet {
    ...
    public String toString() {
        return "["
            + Joiner.on(", ").join(this.all.stream().map(parameter -> (this.required.contains(parameter) ? "!" : "") + parameter.getName()).iterator())
            + "]";
    }

    public void validateUser(ValidationContext lootTableReporter, LootContextUser parameterConsumer) {
        this.validateUser(lootTableReporter.reporter(), parameterConsumer);
    }

    public void validateUser(ProblemReporter errorReporter, LootContextUser parameterConsumer) {
