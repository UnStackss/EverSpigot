...
public class Noises {
    ...
    private static ResourceKey<NormalNoise.NoiseParameters> createKey(String id) {
        return ResourceKey.create(Registries.NOISE, ResourceLocation.withDefaultNamespace(id));
    }

    public static NormalNoise instantiate(
        HolderGetter<NormalNoise.NoiseParameters> noiseParametersLookup, PositionalRandomFactory splitter, ResourceKey<NormalNoise.NoiseParameters> key
    ) {
        Holder<NormalNoise.NoiseParameters> holder = noiseParametersLookup.getOrThrow(key);
        return NormalNoise.create(splitter.fromHashOf(holder.unwrapKey().orElseThrow().location()), holder.value());
    }
}
