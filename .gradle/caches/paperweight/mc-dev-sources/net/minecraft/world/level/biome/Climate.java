...
public class Climate {
    ...
    public static Climate.TargetPoint target(
        ...
        return new Climate.TargetPoint(
            quantizeCoord(temperatureNoise),
            quantizeCoord(humidityNoise),
            quantizeCoord(continentalnessNoise),
            quantizeCoord(erosionNoise),
            quantizeCoord(depth),
            quantizeCoord(weirdnessNoise)
        );
    }
    ...
    interface DistanceMetric<T> {
        long distance(Climate.RTree.Node<T> node, long[] otherParameters);
    }
    ...
    public static class ParameterList<T> {
        ...
        public T findValueIndex(Climate.TargetPoint point) {
            return this.findValueIndex(point, Climate.RTree.Node::distance);
        }

        protected T findValueIndex(Climate.TargetPoint point, Climate.DistanceMetric<T> distanceFunction) {
        ...
        public static final Codec<Climate.ParameterPoint> CODEC = RecordCodecBuilder.create(
            instance -> instance.group(
                        ...
                        Codec.floatRange(0.0F, 1.0F)
                            ...
                            .forGetter(parameterPoint -> parameterPoint.offset)
                    )
    ...
    protected static final class RTree<T> {
        ...
        private static <T> Climate.RTree.Node<T> build(int parameterNumber, List<? extends Climate.RTree.Node<T>> subTree) {
            if (subTree.isEmpty()) {
                ...
                return (Climate.RTree.Node<T>)subTree.get(0);
                ...
                return new Climate.RTree.SubTree<>(list.stream().map(node -> build(parameterNumber, Arrays.asList(node.children))).collect(Collectors.toList()));
            }
        }
        ...
        static final class Leaf<T> extends Climate.RTree.Node<T> {
            ...
            protected Climate.RTree.Leaf<T> search(
                long[] otherParameters, @Nullable Climate.RTree.Leaf<T> alternative, Climate.DistanceMetric<T> distanceFunction
            ) {
                return this;
            }
        }
