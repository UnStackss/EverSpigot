...
public class OreFeature extends Feature<OreConfiguration> {
                            ...
                            for (int ai = ab; ai <= ae; ai++) {
                                ...
                                if (ah * ah + aj * aj < 1.0) {
                                    for (int ak = ac; ak <= af; ak++) {
                                        ...
                                        if (ah * ah + aj * aj + al * al < 1.0 && !world.isOutsideBuildHeight(ai)) {
    ...
    public static boolean canPlaceOre(
        ...
        return target.target.test(state, random) && (shouldSkipAirCheck(random, config.discardChanceOnAirExposure) || !isAdjacentToAir(posToState, pos));
    }
