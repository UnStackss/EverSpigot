...
public class BendingTrunkPlacer extends TrunkPlacer {
    public static final MapCodec<BendingTrunkPlacer> CODEC = RecordCodecBuilder.mapCodec(
        instance -> trunkPlacerParts(instance)
                .and(
                    instance.group(
                        ExtraCodecs.POSITIVE_INT.optionalFieldOf("min_height_for_leaves", 1).forGetter(placer -> placer.minHeightForLeaves),
                        IntProvider.codec(1, 64).fieldOf("bend_length").forGetter(placer -> placer.bendLength)
                    )
                )
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(
        ...
        for (int j = 0; j <= i; j++) {
            ...
            if (TreeFeature.validTreePos(world, mutableBlockPos)) {
                this.placeLog(world, replacer, random, mutableBlockPos, config);
            }
