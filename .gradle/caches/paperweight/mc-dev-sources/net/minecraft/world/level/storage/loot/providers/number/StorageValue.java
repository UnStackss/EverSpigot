...
public record StorageValue(ResourceLocation storage, NbtPathArgument.NbtPath path) implements NumberProvider {
    ...
    private Optional<NumericTag> getNumericTag(LootContext context) {
    ...
    public float getFloat(LootContext context) {
        return this.getNumericTag(context).map(NumericTag::getAsFloat).orElse(0.0F);
    }

    @Override
    public int getInt(LootContext context) {
        return this.getNumericTag(context).map(NumericTag::getAsInt).orElse(0);
    }
}
