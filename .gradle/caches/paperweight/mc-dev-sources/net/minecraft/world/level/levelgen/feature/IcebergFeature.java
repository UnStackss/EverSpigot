...
public class IcebergFeature extends Feature<BlockStateConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<BlockStateConfiguration> context) {
        ...
        for (int t = -o; t < o; t++) {
            for (int u = -o; u < o; u++) {
                for (int v = -1; v > -m; v--) {
                    int w = bl2 ? Mth.ceil((float)o * (1.0F - (float)Math.pow((double)v, 2.0) / ((float)m * 8.0F))) : o;
                    int x = this.heightDependentRadiusSteep(randomSource, -v, m, n);
                    if (t < x) {
                        this.generateIcebergBlock(worldGenLevel, randomSource, blockPos, m, t, v, u, x, w, bl2, j, d, bl, blockState);
                    }
                }
            }
        }
    ...
    private void generateCutOut(RandomSource random, LevelAccessor world, int i, int j, BlockPos pos, boolean bl, int k, double d, int l) {
    ...
    private void generateIcebergBlock(
        ...
        double d = bl
            ? this.signedDistanceEllipse(offsetX, offsetZ, BlockPos.ZERO, j, this.getEllipseC(offsetY, height, k), randomSine)
            : this.signedDistanceCircle(offsetX, offsetZ, BlockPos.ZERO, i, random);
        if (d < 0.0) {
    ...
    private void setIcebergBlock(
        BlockPos pos, LevelAccessor world, RandomSource random, int heightRemaining, int height, boolean lessSnow, boolean placeSnow, BlockState state
    ) {
    ...
    private double signedDistanceCircle(int x, int z, BlockPos pos, int i, RandomSource random) {
    ...
    private int heightDependentRadiusRound(RandomSource random, int y, int height, int factor) {
    ...
    private int heightDependentRadiusSteep(RandomSource random, int y, int height, int factor) {
