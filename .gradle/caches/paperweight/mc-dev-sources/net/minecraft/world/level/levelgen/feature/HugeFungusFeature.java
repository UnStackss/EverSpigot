...
public class HugeFungusFeature extends Feature<HugeFungusConfiguration> {
    ...
    private static boolean isReplaceable(WorldGenLevel world, BlockPos pos, HugeFungusConfiguration config, boolean checkConfig) {
        return world.isStateAtPosition(pos, BlockBehaviour.BlockStateBase::canBeReplaced) || checkConfig && config.replaceableBlocks.test(world, pos);
    }
        ...
        for (int j = -i; j <= i; j++) {
            for (int k = -i; k <= i; k++) {
                ...
                for (int l = 0; l < stemHeight; l++) {
                    ...
                    if (isReplaceable(world, mutableBlockPos, config, true)) {
                        if (config.planted) {
        ...
        for (int k = j; k <= hatHeight; k++) {
            ...
            for (int m = -l; m <= l; m++) {
                for (int n = -l; n <= l; n++) {
                    ...
                    if (isReplaceable(world, mutableBlockPos, config, false)) {
