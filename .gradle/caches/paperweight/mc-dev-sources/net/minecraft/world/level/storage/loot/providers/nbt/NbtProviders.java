...
public class NbtProviders {
    ...
    public static final Codec<NbtProvider> CODEC = Codec.lazyInitialized(
        () -> Codec.either(ContextNbtProvider.INLINE_CODEC, TYPED_CODEC)
                .xmap(
                    Either::unwrap,
                    provider -> provider instanceof ContextNbtProvider contextNbtProvider ? Either.left(contextNbtProvider) : Either.right(provider)
                )
    );
    ...
    private static LootNbtProviderType register(String id, MapCodec<? extends NbtProvider> codec) {
        return Registry.register(BuiltInRegistries.LOOT_NBT_PROVIDER_TYPE, ResourceLocation.withDefaultNamespace(id), new LootNbtProviderType(codec));
    }
}
