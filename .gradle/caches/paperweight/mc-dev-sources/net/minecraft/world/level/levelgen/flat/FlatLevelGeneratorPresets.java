...
public class FlatLevelGeneratorPresets {
    ...
    public static void bootstrap(BootstrapContext<FlatLevelGeneratorPreset> presetRegisterable) {
        new FlatLevelGeneratorPresets.Bootstrap(presetRegisterable).run();
    }

    private static ResourceKey<FlatLevelGeneratorPreset> register(String id) {
        return ResourceKey.create(Registries.FLAT_LEVEL_GENERATOR_PRESET, ResourceLocation.withDefaultNamespace(id));
    }

    static class Bootstrap {
        private final BootstrapContext<FlatLevelGeneratorPreset> context;

        Bootstrap(BootstrapContext<FlatLevelGeneratorPreset> presetRegisterable) {
            this.context = presetRegisterable;
        }

        private void register(
            ResourceKey<FlatLevelGeneratorPreset> registryKey,
            ItemLike icon,
            ...
            FlatLevelGeneratorSettings flatLevelGeneratorSettings = new FlatLevelGeneratorSettings(
                Optional.of(direct), holderGetter3.getOrThrow(biome), FlatLevelGeneratorSettings.createLakesList(holderGetter2)
            );
            if (hasFeatures) {
            ...
            this.context.register(registryKey, new FlatLevelGeneratorPreset(icon.asItem().builtInRegistryHolder(), flatLevelGeneratorSettings));
        }
