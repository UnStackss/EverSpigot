...
public class SetWrittenBookPagesFunction extends LootItemConditionalFunction {
    public static final Codec<Component> PAGE_CODEC = ComponentSerialization.CODEC
        .validate(text -> WrittenBookContent.CONTENT_CODEC.encodeStart(JavaOps.INSTANCE, text).map(value -> text));
    public static final MapCodec<SetWrittenBookPagesFunction> CODEC = RecordCodecBuilder.mapCodec(
        instance -> commonFields(instance)
                .and(
                    instance.group(
                        WrittenBookContent.pagesCodec(PAGE_CODEC).fieldOf("pages").forGetter(function -> function.pages),
                        ListOperation.UNLIMITED_CODEC.forGetter(function -> function.pageOperation)
                    )
                )
    ...
    protected SetWrittenBookPagesFunction(List<LootItemCondition> conditions, List<Filterable<Component>> pages, ListOperation operation) {
        super(conditions);
        this.pages = pages;
    ...
    @VisibleForTesting
    public WrittenBookContent apply(WrittenBookContent current) {
        List<Filterable<Component>> list = this.pageOperation.apply(current.pages(), this.pages);
