...
public class LargeDripstoneFeature extends Feature<LargeDripstoneConfiguration> {
    ...
    private static LargeDripstoneFeature.LargeDripstone makeDripstone(
        BlockPos pos, boolean isStalagmite, RandomSource randomSource, int scale, FloatProvider bluntness, FloatProvider heightScale
    ) {
        return new LargeDripstoneFeature.LargeDripstone(
            pos, isStalagmite, scale, (double)bluntness.sample(randomSource), (double)heightScale.sample(randomSource)
        );
    }
    ...
    private void placeDebugMarkers(WorldGenLevel world, BlockPos pos, Column.Range surface, LargeDripstoneFeature.WindOffsetter wind) {
        ...
        for (BlockPos.MutableBlockPos mutableBlockPos = pos.atY(surface.floor() + 2).mutable();
            mutableBlockPos.getY() < surface.ceiling() - 1;
    ...
    static final class LargeDripstone {
        ...
        void placeBlocks(WorldGenLevel world, RandomSource random, LargeDripstoneFeature.WindOffsetter wind) {
                    ...
                    if (!(f > (float)this.radius)) {
                        ...
                        if (k > 0) {
                            ...
                            int l = this.pointingUp
                                ? world.getHeight(Heightmap.Types.WORLD_SURFACE_WG, mutableBlockPos.getX(), mutableBlockPos.getZ())
                                : Integer.MAX_VALUE;
