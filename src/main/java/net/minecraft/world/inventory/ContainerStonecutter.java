...
public class ContainerStonecutter extends Container {
    ...
    private List<RecipeHolder<RecipeStonecutting>> recipes;
    ...
    private void setupRecipeList(IInventory iinventory, ItemStack itemstack) {
        ...
        if (!itemstack.isEmpty()) {
            this.recipes = this.level.getRecipeManager().getRecipesFor(Recipes.STONECUTTING, createRecipeInput(iinventory), this.level);
        }
    ...
    void setupResultSlot() {
        if (!this.recipes.isEmpty() && this.isValidRecipeIndex(this.selectedRecipeIndex.get())) {
            RecipeHolder<RecipeStonecutting> recipeholder = (RecipeHolder) this.recipes.get(this.selectedRecipeIndex.get());
            ItemStack itemstack = ((RecipeStonecutting) recipeholder.value()).assemble(createRecipeInput(this.container), this.level.registryAccess());
    ...
    public ItemStack quickMoveStack(EntityHuman entityhuman, int i) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (this.level.getRecipeManager().getRecipeFor(Recipes.STONECUTTING, new SingleRecipeInput(itemstack1), this.level).isPresent()) {
