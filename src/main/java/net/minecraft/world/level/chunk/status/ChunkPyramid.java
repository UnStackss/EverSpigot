...
public record ChunkPyramid(ImmutableList<ChunkStep> steps) {
    public static final ChunkPyramid GENERATION_PYRAMID = new ChunkPyramid.Builder()
        ...
        .step(
            ...
            builder -> builder.addRequirement(ChunkStatus.STRUCTURE_STARTS, 8).setTask(ChunkStatusTasks::generateStructureReferences)
        )
        ...
        .step(
            ChunkStatus.CARVERS,
            builder -> builder.addRequirement(ChunkStatus.STRUCTURE_STARTS, 8).blockStateWriteRadius(0).setTask(ChunkStatusTasks::generateCarvers)
        )
        .step(
    ...
    public static class Builder {
        ...
        public ChunkPyramid.Builder step(ChunkStatus status, UnaryOperator<ChunkStep.Builder> stepFactory) {
            ChunkStep.Builder builder;
            if (this.steps.isEmpty()) {
                builder = new ChunkStep.Builder(status);
            } else {
                builder = new ChunkStep.Builder(status, this.steps.getLast());
            }
