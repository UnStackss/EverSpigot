...
public record GameEvent(int notificationRadius) {
    ...
    public static final class ListenerInfo implements Comparable<GameEvent.ListenerInfo> {
        ...
        public ListenerInfo(Holder<GameEvent> event, Vec3 emitterPos, GameEvent.Context emitter, GameEventListener listener, Vec3 listenerPos) {
            this.gameEvent = event;
            this.source = emitterPos;
            this.context = emitter;
            this.recipient = listener;
            this.distanceToRecipient = emitterPos.distanceToSqr(listenerPos);
        }
