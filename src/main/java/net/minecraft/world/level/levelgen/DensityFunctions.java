...
public final class DensityFunctions {
    ...
    private static MapCodec<? extends DensityFunction> register(
        Registry<MapCodec<? extends DensityFunction>> registry, String id, KeyDispatchDataCodec<? extends DensityFunction> codecHolder
    ) {
        ...
        return KeyDispatchDataCodec.of(
            RecordCodecBuilder.mapCodec(
                instance -> instance.group(
                            DensityFunction.HOLDER_HELPER_CODEC.fieldOf("argument1").forGetter(argument1Getter),
                            DensityFunction.HOLDER_HELPER_CODEC.fieldOf("argument2").forGetter(argument2Getter)
                        )
    ...
    public static DensityFunction mappedNoise(
        Holder<NormalNoise.NoiseParameters> noiseParameters, @Deprecated double scaleXz, double scaleY, double min, double max
    ) {
    ...
    public static DensityFunction mappedNoise(Holder<NormalNoise.NoiseParameters> noiseParameters, double scaleY, double min, double max) {
        return mappedNoise(noiseParameters, 1.0, scaleY, min, max);
    }

    public static DensityFunction mappedNoise(Holder<NormalNoise.NoiseParameters> noiseParameters, double min, double max) {
        return mappedNoise(noiseParameters, 1.0, 1.0, min, max);
    }

    public static DensityFunction shiftedNoise2d(
        DensityFunction shiftX, DensityFunction shiftZ, double xzScale, Holder<NormalNoise.NoiseParameters> noiseParameters
    ) {
        return new DensityFunctions.ShiftedNoise(shiftX, zero(), shiftZ, xzScale, 0.0, new DensityFunction.NoiseHolder(noiseParameters));
    }
    ...
    public static DensityFunction weirdScaledSampler(
        DensityFunction input, Holder<NormalNoise.NoiseParameters> parameters, DensityFunctions.WeirdScaledSampler.RarityValueMapper mapper
    ) {
    ...
    static record Ap2(
        ...
        public double compute(DensityFunction.FunctionContext pos) {
            ...
            return switch (this.type) {
                ...
                case MUL -> d == 0.0 ? 0.0 : d * this.argument2.compute(pos);
            ...
            switch (this.type) {
                ...
                case MUL:
                    for (int j = 0; j < densities.length; j++) {
                        ...
                        densities[j] = d == 0.0 ? 0.0 : d * this.argument2.compute(applier.forIndex(j));
                    }
                    break;
                case MIN:
    ...
    static record BlendDensity(@Override DensityFunction input) implements DensityFunctions.TransformerWithContext {
    ...
    protected static record Clamp(@Override DensityFunction input, @Override double minValue, @Override double maxValue)
    ...
    protected static final class EndIslandDensityFunction implements DensityFunction.SimpleFunction {
        ...
        @Override
        public double compute(DensityFunction.FunctionContext pos) {
            return ((double)getHeightValue(this.islandNoise, pos.blockX() / 8, pos.blockZ() / 8) - 8.0) / 128.0;
        }
    ...
    protected static record Mapped(DensityFunctions.Mapped.Type type, @Override DensityFunction input, @Override double minValue, @Override double maxValue)
        implements DensityFunctions.PureTransformer {
        ...
        private static double transform(DensityFunctions.Mapped.Type type, double density) {
            return switch (type) {
                ...
                case SQUEEZE -> {
                    double d = Mth.clamp(density, -1.0, 1.0);
    ...
    protected static record Marker(@Override DensityFunctions.Marker.Type type, @Override DensityFunction wrapped) implements DensityFunctions.MarkerOrMarked {
        @Override
        public double compute(DensityFunction.FunctionContext pos) {
    ...
    protected static record Shift(@Override DensityFunction.NoiseHolder offsetNoise) implements DensityFunctions.ShiftNoise {
    ...
    protected static record ShiftA(@Override DensityFunction.NoiseHolder offsetNoise) implements DensityFunctions.ShiftNoise {
        ...
        @Override
        public double compute(DensityFunction.FunctionContext pos) {
            return this.compute((double)pos.blockX(), 0.0, (double)pos.blockZ());
        }
    ...
    protected static record ShiftB(@Override DensityFunction.NoiseHolder offsetNoise) implements DensityFunctions.ShiftNoise {
        ...
        @Override
        public double compute(DensityFunction.FunctionContext pos) {
            return this.compute((double)pos.blockZ(), (double)pos.blockX(), 0.0);
        }
    ...
    public static record Spline(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> spline) implements DensityFunction {
        ...
        @Override
        public DensityFunction mapAll(DensityFunction.Visitor visitor) {
            return visitor.apply(new DensityFunctions.Spline(this.spline.mapAll(densityFunctionWrapper -> densityFunctionWrapper.mapAll(visitor))));
        }
    ...
    interface TwoArgumentSimpleFunction extends DensityFunction {
        ...
        static DensityFunctions.TwoArgumentSimpleFunction create(
            ...
            if (type == DensityFunctions.TwoArgumentSimpleFunction.Type.MUL || type == DensityFunctions.TwoArgumentSimpleFunction.Type.ADD) {
                ...
                if (argument2 instanceof DensityFunctions.Constant constant2) {
                    return new DensityFunctions.MulOrAdd(
                        type == DensityFunctions.TwoArgumentSimpleFunction.Type.ADD ? DensityFunctions.MulOrAdd.Type.ADD : DensityFunctions.MulOrAdd.Type.MUL,
                        argument1,
                        h,
