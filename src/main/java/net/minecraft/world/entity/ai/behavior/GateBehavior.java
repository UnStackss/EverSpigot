...
public class GateBehavior<E extends LivingEntity> implements BehaviorControl<E> {
    ...
    public GateBehavior(
        ...
        GateBehavior.OrderPolicy order,
        GateBehavior.RunningPolicy runMode,
        ...
        tasks.forEach(task -> this.behaviors.add((BehaviorControl<? super E>)task.getFirst(), task.getSecond()));
    }
    ...
    public String toString() {
        Set<? extends BehaviorControl<? super E>> set = this.behaviors
            .stream()
            .filter(task -> task.getStatus() == Behavior.Status.RUNNING)
    ...
    public static enum OrderPolicy {
        ...
        private OrderPolicy(final Consumer<ShufflingList<?>> listModifier) {
